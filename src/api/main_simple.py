...
async def get_weather_forecast(city: str, days: int = 7):
    ...
    if email_notifier and days >= 7:
        email_notifier.send_weekly_forecast("marcoszambon18@gmail.com", city, forecast)
...
async def startup_event():
    logger.info("WeatherAlert API iniciando...")
    
    desktop_notifier.send_weather_alert(
        "WeatherAlert Iniciado",
        f"Sistema ativo!\nMonitorando: {', '.join(DEFAULT_CITIES)}"
    )
    
    logger.info("WeatherAlert API pronta para uso!")
